...
var hasError = function (field) {
	...
	if (validity.valueMissing) return validationMessages.valueMissing || 'Please fill out this field.';

	// If not the right type
	if (validity.typeMismatch) {

		// Email
		if (field.type === 'email') return validationMessages.typeMismatchEmail || 'Please enter an email address.';

		// URL
		if (field.type === 'url') return validationMessages.typeMismatchURL || 'Please enter a URL.';
	...
	if (validity.patternMismatch) {
		...
		return validationMessages.patternMismatch || 'Please match the requested format.';

	}

	// If number input isn't a number
	if (validity.badInput) return validationMessages.badInput || 'Please enter a number.';

	// If a number value doesn't match the step interval
	if (validity.stepMismatch) return validationMessages.stepMismatch || 'Please select a valid value.';
	...
	return validationMessages.general || 'The value you entered for this field is invalid.';
...
window.displayMailChimpStatus = function (data) {
	...
	mcStatus.id = 'mce-' + data.result + '-response';

	if ( /^\d+\s\-/.test(data.msg) ) {
